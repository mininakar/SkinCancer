...
function ive_render_custom_text_field($post, $meta_data) {
  ...
  foreach ($meta_data['fields'] as $data) {
    ...
    if ($data['f_post_type'] == get_post_type($post)) {
     ...
     if($data['f_type'] == 'text'){
      ...
      ?>
  <label for="custom-text-field"><?php echo $f_title . ': '; ?></label>
  <br>
  <input type="text" id="custom-text-field" name="<?php echo $f_name;?>" value="<?php echo esc_attr($custom_text_value); ?>" style="width:90%;" />
  <br>
  <br>
  <?php
  }
  ...
  <label for="custom_checkbox">
    <?php _e($f_title . ': ', 'text-domain'); ?>
      <input type="checkbox" name="<?php echo $f_name;?>" id="<?php echo $f_name;?>" value="1" <?php checked($value, 1); ?>>
  </label>
  <br>
  <?php
  }
    			...
    			if( $custom_repeater_item ){
    				foreach( $custom_repeater_item as $item_key => $item_value ){?>
    					<tr class="ive-wc-sub-row">
    						<td>
    							<input name="<?php echo $f_name.'[' .$item_key. ']';?>[title]" type="text" value="<?php echo (isset($item_value['title'])) ? $item_value['title'] : ''; ?>" style="width:98%;" placeholder="Heading">
    						</td>
    						<td>
    							<input type="text" name="<?php echo $f_name.'[' .$item_key. ']';?>[desc]" value="<?php echo (isset($item_value['desc'])) ? $item_value['desc'] : ''; ?>" style="width:98%;" placeholder="Description"/>
    						</td>
    						<td>
    							<button class="ive-wc-remove-item button" type="button">Remove</button>
    						</td>
    					</tr><?php
    				}
    			}
          ...
          <p> <?php echo($f_title . ': ');?></p>
    			<tr class="ive-wc-hide-tr" style="display: none;">
    				<td>
    					...
    					<input class="ive-get-hidden-val" type="hidden" value="<?php echo $f_name ?>">
    				</td>
    				<td>
    		...
    		<tfoot>
    			<tr>
    				<td colspan="4"><button class="ive-wc-add-item button" type="button">Add another</button></td>
    			</tr>
    		</tfoot>
    	</table>
    	<?php
...
function ive_save_custom_text_field($post_id) {
  ...
  foreach ($get_all_data as $datas) {
    ...
    if($data['f_type'] == 'checkbox'){
      $f_name = $data['f_name'];
          // if (!isset($_POST['custom_checkbox_field_nonce']) || !wp_verify_nonce($_POST['custom_checkbox_field_nonce'], 'custom_checkbox_field')) {
          //     return;
          // }
          if (isset($_POST[$f_name])) {
      ...
      if ( isset($_POST[$f_name]) ){
      update_post_meta( $post_id, $f_name, $_POST[$f_name] );
      } else {
